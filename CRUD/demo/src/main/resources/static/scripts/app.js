...
/*btnBorrarTarea.addEventListener('click', async () => {
    const checkboxes = contenedorTarea.querySelectorAll('.tarea-acciones input[type="checkbox"]');
    const checkedTareas = Array.from(checkboxes).filter(checkbox => checkbox.checked);

    if (checkedTareas.length === 0) {
        alert('No hay tareas seleccionadas para eliminar.');
        return;
    }

    for (const checkbox of checkedTareas) {
        const contenedorPadre = checkbox.closest('.tarea-item');
        const idTarea = contenedorPadre.dataset.idTarea; 
...
/*document.getElementById("btnBorrarTarea").addEventListener("click", () => {
    //Seleccionar los checkbox dentro de la tarea
    const checkboxes = contenedorTarea.querySelectorAll('.tarea-acciones input[type = "checkbox"]');
    
    //Iterar cada checkbox para checar si están seleccionados
    checkboxes.forEach(checkbox => {
        //Si está seleccionado, obtener el div padre que contiene toda la tarea (.tarea-item) y borrarlo del DOM
        if(checkbox.checked) {
            const contenedorPadre = checkbox.closest('.tarea-item');
